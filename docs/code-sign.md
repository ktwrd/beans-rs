...
This repo uses [`skymatic/code-sign-action`](https://github.com/marketplace/actions/windows-signtool-exe-code-sign-action) to sign the latest builds of `beans`. The most important components of this action are `certificate`, `certificatesha1` and `password`.
